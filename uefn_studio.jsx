...
/* ─── FONTS + GLOBAL CSS ─── */
const FONTS = `@import url('https://fonts.googleapis.com/css2?family=Rajdhani:wght@500;600;700&family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,600&display=swap');`;
const GCSS = `
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
html{scroll-behavior:smooth}
body{font-family:'DM Sans',sans-serif;background:#060610;color:#eaeaf5}
textarea,input,select{font-family:'DM Sans',sans-serif}
@keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}}
@keyframes shimmer{0%{background-position:200% center}100%{background-position:-200% center}}
.fu{animation:fadeUp .3s ease forwards}
.fu2{animation:fadeUp .3s .08s ease both}
.fu3{animation:fadeUp .3s .16s ease both}
...
/* ─── STATIC CONSTANTS (structural, not user-editable) ─── */
const COMPL = [
...
/* ─── ICON MAP (for admin icon picker) ─── */
...
const DEF_ORDERS = [
  { id:"UEFN-K4X9", discord:"Blaze#4521",    type:"deathrun",  complexity:"complex", timeline:"priority", addons:["hud","verse"], desc:"50-stage deathrun, moving platforms, global leaderboard.", status:"in_progress", price:{min:1750,max:2100}, createdAt:new Date(Date.now()-5*864e5).toISOString() },
  { id:"UEFN-M7T2", discord:"FortKing#9201", type:"prop_hunt", complexity:"medium",  timeline:"standard", addons:["audio"],       desc:"Classic prop hunt on modern city map.",                    status:"pending",     price:{min:650,max:775},  createdAt:new Date(Date.now()-2*864e5).toISOString() },
];
const ADMIN_PASS = "studio2025";
...
const genId = ()  => `UEFN-${Math.random().toString(36).slice(2,6).toUpperCase()}`;
...
export default function App() {
  ...
  const [orders, _setOrders]  = useState(DEF_ORDERS);
  ...
  const [newPort, setNewPort] = useState({title:"",cat:"",code:"",plays:"",rating:"5.0",iconKey:"Wind",gradIdx:0});
  ...
  /* — persist helpers — */
  const setTypes  = async v => { _setTypes(v);   try { await window.storage.set("uefn_types",    JSON.stringify(v)); } catch {} };
  const setAddons = async v => { _setAddons(v);  try { await window.storage.set("uefn_addons",   JSON.stringify(v)); } catch {} };
  const setPort   = async v => { _setPort(v);    try { await window.storage.set("uefn_portfolio", JSON.stringify(v)); } catch {} };
  const setOrders = async v => { _setOrders(v);  try { await window.storage.set("uefn_orders",   JSON.stringify(v)); } catch {} };
  const setBookingP = async v => { setBooking(v); try { await window.storage.set("uefn_open", JSON.stringify(v)); } catch {} };

  /* — load from storage — */
  useEffect(() => {
    const s = document.createElement("style");
    s.textContent = FONTS + GCSS;
    document.head.appendChild(s);
    (async () => {
      try { const r = await window.storage.get("uefn_orders");    if (r) _setOrders(JSON.parse(r.value)); } catch {}
      try { const r = await window.storage.get("uefn_types");     if (r) _setTypes(JSON.parse(r.value));  } catch {}
      try { const r = await window.storage.get("uefn_addons");    if (r) _setAddons(JSON.parse(r.value)); } catch {}
      try { const r = await window.storage.get("uefn_portfolio"); if (r) _setPort(JSON.parse(r.value));   } catch {}
      try { const r = await window.storage.get("uefn_open");      if (r) setBooking(JSON.parse(r.value)); } catch {}
    })();
    return () => document.head.removeChild(s);
  }, []);

  /* — portfolio carousel auto-cycle — */
  ...
  async function submitOrder() {
    ...
    const order = { id:genId(), discord:contact.discord.trim(), email:contact.email.trim(), type:quiz.type, complexity:quiz.complexity, timeline:quiz.timeline, addons:[...quiz.addons], desc:contact.desc.trim(), notes:contact.notes.trim(), status:"pending", price, createdAt:new Date().toISOString() };
  ...
  async function advance(id) { await setOrders(orders.map(o => o.id===id && STATUS_FLOW[o.status] ? {...o,status:STATUS_FLOW[o.status]} : o)); }
  async function cancelOrd(id) { await setOrders(orders.map(o => o.id===id ? {...o,status:"cancelled"} : o)); }
  ...
  function addPort() {
    ...
    setPort([...portfolio, { id:"p"+nid(), order:maxOrder+1, visible:true, cat:newPort.cat||"Maps", title:newPort.title, code:newPort.code||"0000-0000-0000", plays:newPort.plays||"0", rating:parseFloat(newPort.rating)||5.0, grad:GRADS[newPort.gradIdx]||GRADS[0], iconKey:newPort.iconKey||"Wind" }]);
    setNewPort({title:"",cat:"",code:"",plays:"",rating:"5.0",iconKey:"Wind",gradIdx:0});
      ...
      <div style={{maxWidth:980,margin:"0 auto",padding:"0 20px"}}>
                ...
                {curPortItem && (
                  <div key={`${cycleIdx}-${curPortItem.id}`} className="fu" style={{borderRadius:16,overflow:"hidden",border:`1px solid ${C.b1}`,background:C.s0}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",minHeight:220}}>
                      {/* Visual panel */}
                      <div style={{background:`linear-gradient(135deg,${curPortItem.grad[0]},${curPortItem.grad[1]})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,padding:28,position:"relative"}}>
                        {(() => { const Ic = ICON_MAP[curPortItem.iconKey]||Wind; return <Ic size={52} color="rgba(255,255,255,.9)"/>; })()}
        ...
        {view==="track" && (
          <div className="fu" style={{maxWidth:520,margin:"30px auto"}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:18}}>
              <input value={trackId} onChange={e=>setTrackId(e.target.value)} onKeyDown={e=>e.key==="Enter"&&runTrack()} placeholder="UEFN-XXXX"
        ...
        {view==="admin" && !authed && (
          <div className="fu" style={{maxWidth:340,margin:"50px auto",textAlign:"center"}}>
            ...
            <h2 style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:22,marginBottom:6}}>Owner access</h2>
            <p style={{color:C.mu,fontSize:13,marginBottom:18}}>Demo passcode: <code style={{color:C.a}}>studio2025</code></p>
            <input type="password" value={passIn} onChange={e=>{setPassIn(e.target.value);setPassErr(false);}} onKeyDown={e=>e.key==="Enter"&&(passIn===ADMIN_PASS?(setAuthed(true),setPassErr(false)):setPassErr(true))} placeholder="Passcode"
              style={{width:"100%",background:"rgba(255,255,255,.04)",border:`1px solid ${passErr?C.re:C.b0}`,borderRadius:8,color:C.tx,fontSize:15,padding:"11px 14px",outline:"none",marginBottom:8,textAlign:"center"}}/>
            {passErr && <div style={{color:C.re,fontSize:12,marginBottom:8}}>Incorrect passcode.</div>}
            <button onClick={()=>passIn===ADMIN_PASS?setAuthed(true):setPassErr(true)} style={{width:"100%",padding:11,borderRadius:8,fontSize:14,fontWeight:600,cursor:"pointer",background:C.p,border:"none",color:"#fff",fontFamily:"inherit"}}>Unlock</button>
          </div>
        )}
        ...
        {view==="admin" && authed && (
          <div className="fu" style={{paddingTop:24,paddingBottom:40}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
              ...
              <button onClick={()=>{setAuthed(false);setPassIn("");}} style={{display:"flex",alignItems:"center",gap:5,padding:"7px 11px",borderRadius:8,fontSize:12,fontWeight:600,cursor:"pointer",background:"transparent",border:`1px solid ${C.b0}`,color:C.mu,fontFamily:"inherit"}}><LogOut size={13}/></button>
            </div>

            {/* Stats */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:9,marginBottom:18}}>
              {[[orders.length,"Orders",C.tx,<LayoutDashboard size={14}/>],[stats.active,"Active",C.p,<TrendingUp size={14}/>],[stats.pending,"Pending",C.gold,<Clock size={14}/>],[fmt(stats.rev),"Revenue",C.gr,<DollarSign size={14}/>]].map(([v,k,col,ic])=>(
            ...
            {dashTab==="portfolio" && (
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  <p style={{color:C.mu,fontSize:13}}>Drag order controls affect the carousel cycle on the studio page.</p>
                ...
                {showNewPort && (
                  <div style={{background:C.s1,border:`1px solid ${C.pBd}`,borderRadius:12,padding:18,marginBottom:14}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                      {[["Title","title","text"],["Category","cat","text"],["Island Code","code","text"],["Plays (e.g. 412K)","plays","text"],["Rating (0–5)","rating","text"]].map(([lbl,field,type])=>(
                  ...
                  {[...portfolio].sort((a,b)=>a.order-b.order).map(p=>{
                    ...
                    return (
                      <div key={p.id} style={{background:C.s0,border:`1px solid ${C.b0}`,borderRadius:10,padding:"11px 14px",display:"flex",alignItems:"center",gap:12,opacity:p.visible?1:.5}}>
                        <div style={{width:36,height:36,borderRadius:8,background:`linear-gradient(135deg,${p.grad[0]},${p.grad[1]})`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Ic size={16} color="rgba(255,255,255,.9)"/></div>
                        <div style={{flex:1,minWidth:0}}>
